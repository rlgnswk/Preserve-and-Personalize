...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Weight for L2 regularization toward the pretrained initialization."
    )
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
...
def main(args):
    ...
    if args.seed is not None:
        ...
        # Support NPU (Ascend) if available
        try:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                for p in trainable_params:              # Use the current trainable-parameter list
                    init   = w_init[id(p)].to(p.device)
                    reg_loss += F.mse_loss(p, init)
                    #numel   += p.numel()
                # reg_loss = reg_loss / numel             # Mean MSE over parameters
                loss_recon = loss
                loss_reg = reg_loss * args.l2_weight
            ...
            logs = {"loss_recon": loss_recon.detach().item(), "loss_reg": loss_reg.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
