...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--l2_weight",
        type=float,
        default=0.0,         # 필요하면 required=True로 바꿔도 됩니다
        help="L2 regularization coefficient applied to LoRA (or trainable) params."
    )
    parser.add_argument(
        "--revision",
        type=str,
...
def main(args):
    ...
    if args.seed is not None:
        ...
        # TF32 끄기 (Ampere 이상 reproducibility 위해)
    ...
    if args.with_prior_preservation:
        ...
        prior_end = time.time()
        prior_elapsed = prior_end - prior_start

        if accelerator.is_main_process:
            os.makedirs(args.output_dir, exist_ok=True)
            with open(os.path.join(args.output_dir, "timing.txt"), "a") as f:
                f.write(f"Prior generation elapsed: {prior_elapsed:.2f} sec\n")
        ...
        with TorchTracemalloc() if not args.no_tracemalloc else nullcontext() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    l2_reg = torch.zeros((), device=accelerator.device)
                    if args.l2_weight != 0.0:
                        for name, p in unet.named_parameters():  # unet 안에서
                            if p.requires_grad and "lora_" in name.lower():
                                l2_reg += torch.sum(p ** 2)
                            
                    recon_loss = loss
                    
                    loss = loss + l2_reg * args.l2_weight #* 0.000000001 
                ...
                logs = {"loss": loss.detach().item(), "recon_loss": recon_loss.detach().item(), "l2_reg": l2_reg.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
    ...
    if accelerator.is_main_process:
        with open(os.path.join(args.output_dir, "timing.txt"), "a") as f:
            f.write(f"Finetune training elapsed: {train_elapsed:.2f} sec\n")
