...
PRIOR_WEIGHT   = 100    # Weight of the prior term in the multitask loss
...
# ---------- Output directory ----------
fig_dir = "toy_db"; os.makedirs(fig_dir, exist_ok=True)
...
# Connect the total number of classes and hidden size to the model
...
x, y = make_pentagon_mog(n_per_class=N_PER_CLASS)  # ★ N_PER_CLASS
X, Y = x.to(device), y.to(device)

# ---------- 2) New class for fine-tuning (origin) ----------
mean_new = torch.tensor([0.0, 0.0])
std_new  = 0.5
X_new    = mean_new + std_new*torch.randn(N_PER_CLASS, 2)  # ★ N_PER_CLASS
Y_new    = torch.full((N_PER_CLASS,), NEW_CLASS_IDX, dtype=torch.long)  # ★ NEW_CLASS_IDX
...
for cls in range(N_BASE_CLASSES):
...
# ---------- 8) Prepare fine-tuning data separately for new and prior ----------
mask0   = (Y.cpu() == 0)
X_prior = X.cpu()[mask0]                          # (N0, 2)
Y_prior = torch.zeros_like(Y.cpu()[mask0])        # Label 0

# ---------- 9) Fine-tuning (multitask: new + PRIOR_WEIGHT * prior) ----------
loss_history = []
batch_size = 512

for _ in tqdm(range(5000), desc="Fine-tune multitask"):
    idx_new   = torch.randperm(len(X_new))
    idx_prior = torch.randperm(len(X_prior))
    for i in range(0, len(X_new), batch_size):
        # new batch
        xb_new = X_new[idx_new[i:i+batch_size]].to(device)
        yb_new = Y_new[idx_new[i:i+batch_size]].to(device)
        tb_new = torch.randint(0, T, (len(xb_new),1), device=device)
        x_noised_new, noise_new = q_sample(xb_new, tb_new.squeeze(), alpha_bars)
        pred_new = model(x_noised_new, tb_new, yb_new)
        loss_new = loss_fn(pred_new, noise_new)

        # Prior batch of the same size
        xb_pr = X_prior[idx_prior[i:i+batch_size]].to(device)
        yb_pr = Y_prior[idx_prior[i:i+batch_size]].to(device)
        tb_pr = torch.randint(0, T, (len(xb_pr),1), device=device)
        x_noised_pr, noise_pr = q_sample(xb_pr, tb_pr.squeeze(), alpha_bars)
        pred_pr = model(x_noised_pr, tb_pr, yb_pr)
        loss_prior = loss_fn(pred_pr, noise_pr)

        # Multitask loss (kept as in the original setup)
        loss = loss_new + loss_prior * PRIOR_WEIGHT

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        loss_history.append((loss_new.item(), loss_prior.item(), loss.item()))

# ----- Loss plot disabled: do not generate or save finetune_loss.png -----
# ln, lp, lt = zip(*loss_history)  # new, prior, total
# plt.figure()
# plt.plot(ln, label="new")
# plt.plot(lp, label=f"prior (x{PRIOR_WEIGHT})")
# plt.plot(lt, label="total")
# plt.title("Finetune Loss (new / prior / total)")
# plt.xlabel("Iteration"); plt.ylabel("MSE"); plt.legend(); plt.grid(True); plt.tight_layout()
# plt.savefig(f"{fig_dir}/finetune_loss.png", dpi=300); plt.show()

# 10) Personalized samples
plt.figure(figsize=(6,6))
