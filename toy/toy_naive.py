...
N_BASE_CLASSES  = 5      # Number of base training classes (default: pentagon)
N_UNITS         = 96     # Hidden size
# Assume that one new class is added at the origin (keep the original setup)
NEW_CLASS_IDX      = N_BASE_CLASSES
N_CLASSES_TOTAL    = N_BASE_CLASSES + 1  # Initial N classes + 1 new class
...
# ---------- Output directory ----------
fig_dir = "toy_naive"; os.makedirs(fig_dir, exist_ok=True)
...
class ConditionalDiffusionModel(nn.Module):
    def __init__(self, nfeatures=2, nclasses=6, nunits=32):  # Keep the default `nunits=32`
        super().__init__()
...
# Connect `nclasses` and `nunits` to the top-level hyperparameters
...
# ---------- Colors for visualization (keep the original palette) ----------
colors = [
    "#0072B2",  # Class 0 - blue
...
assert N_CLASSES_TOTAL <= len(colors), \
    f"The current color palette has {len(colors)} colors. Extend `colors` to support N_CLASSES_TOTAL={N_CLASSES_TOTAL}."
...
for cls in range(N_BASE_CLASSES):  # ★ 5 -> N_BASE_CLASSES
    ...
    plt.scatter(pts[:,0], pts[:,1], alpha=.8, color=colors[cls])
setup_axes()
...
setup_axes("Pretrained Samples")
plt.savefig(f"{fig_dir}/pretrained_samples.png", dpi=300); plt.show()

# 7) Target data
plt.figure(figsize=(6,6))
plt.scatter(new_pts[:,0], new_pts[:,1], alpha=.8, color=colors[NEW_CLASS_IDX])  # Replace fixed index 5 with NEW_CLASS_IDX
setup_axes("Target Data")
plt.savefig(f"{fig_dir}/target_data.png", dpi=300); plt.show()

# 8) Fine-tuning (new class)
loss_history = []
...
# plt.figure(); plt.plot(loss_history); plt.title("Finetune Loss (Class 5)")
