...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Explicit L2 penalty coefficient applied to LoRA parameters in the UNet. "
            "Set to a value > 0 to enable (e.g., 1e-4)."
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
...
def main(args):
    ...
    if args.seed is not None:
        ...
        # Support NPU (Ascend) when available.
        try:
        ...
        # Disable TF32 for reproducibility on Ampere or newer GPUs.
        if hasattr(torch.backends.cuda, "matmul"):
            torch.backends.cuda.matmul.allow_tf32 = False
        if hasattr(torch.backends.cudnn, "allow_tf32"):
            torch.backends.cudnn.allow_tf32 = False

        # Fix the Python hash seed.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                l2_reg = torch.zeros((), device=loss.device, dtype=loss.dtype)
                if args.l2_reg_weight > 0.0:
                    for name, p in unet.named_parameters():  # Only regularize LoRA parameters in the UNet.
                        if p.requires_grad and "lora_" in name.lower():
                            l2_reg += torch.sum(p ** 2)
                            
                recon_loss = loss
                reg_loss = l2_reg * args.l2_reg_weight
            ...
            logs = {"loss": loss.detach().item(), "recon_loss": recon_loss.detach().item(), "reg_loss": reg_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
    ...
    if accelerator.is_main_process:
        ...
        for i, image in enumerate(images):
            save_path = os.path.join(args.output_dir, f"val_epoch{epoch}_{i}.png")
