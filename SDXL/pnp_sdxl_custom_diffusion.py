...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="L2 regularization weight applied to the trainable parameters.",
    )
    parser.add_argument(
...
def main(args):
    ...
    #accelerator.scaler = GradScaler(enabled=False)
    ...
    # ---------- Save a copy of the initial trainable weights ----------
    trainable_params = list(params_to_optimize)
    w_init = {id(p): p.detach().clone() for p in trainable_params}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # ---------- Regularization ----------
                reg_loss, numel = 0.0, 0
                for p in trainable_params:              # Use the explicit trainable parameter list.
                    init   = w_init[id(p)].to(p.device)
                    reg_loss += F.mse_loss(p, init)
                    #numel   += p.numel()
                # reg_loss = reg_loss / numel             # Average MSE across trainable parameters.
                loss = loss + reg_loss * args.l2_reg_weight
    ...
    # -------------- Save fine‑tuned UNet only --------------
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        # Convert to fp32 before saving.
        tuned_unet = accelerator.unwrap_model(unet).to(torch.float32)

        # Save in diffusers format (config + weights).
        unet_save_dir = os.path.join(args.output_dir, "unet")
        tuned_unet.save_pretrained(unet_save_dir)          # Saving the UNet directory is sufficient here.
